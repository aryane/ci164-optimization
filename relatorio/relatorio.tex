...
os tamanhos de cache exibidos acima. Já para matrizes de dimensões maiores, que
fogem ao escopo do trabalho, com a arquitetura utilizada, a partir de x é
possível verificar uma queda no desempenho medido em segundos para matrizes de
dimensão potência de $2$ em relacao às outras.  Uma melhor visualização dos
tempos de execução pode ser observado na tabela~\ref{fig:tabelaExecucoes}.
...
%n, há $2n^2$ flop.  Na função multVet, há dois flop's dentro de um laço, então a
%função é 2n. Como a função lambda usa duas vezes a função multVet e uma vez a
%função multMat, flops em função de n é, aproximadamente, f(n) = $2n^2 + 4n$,
...
%paralelos.
%
%\subsection{Otimização: Loop blocking}
%
\end{document}
