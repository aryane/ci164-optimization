...
all_opt:
	        $(CC) src/main.c src/gradOpt.c src/grad.h src/mat_utils.* -o bin/gradSolver $(CFLAGS)

lambda:
			$(CC) src/gradLambda.c src/grad.* src/mat_utils.* -o bin/gradLambda $(CFLAGS) $(LIKWIDFLAGS)

lambda_opt:
			$(CC) src/gradLambdaOpt.c src/gradOpt.c src/grad.h src/mat_utils.* -o bin/gradLambda $(CFLAGS) $(LIKWIDFLAGS)
